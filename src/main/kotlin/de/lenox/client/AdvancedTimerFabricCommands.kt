...
object AdvancedTimerFabricCommands {
    fun register(dispatcher: CommandDispatcher<FabricClientCommandSource>) {
        val timeCommand = literal("time")
        listOf("add", "subtract", "set").forEach { action ->
            timeCommand.then(
                literal(action).then(
                    argument("seconds", IntegerArgumentType.integer(0)).executes { ctx ->
                        handleTimeAction(ctx, action, false, false, false)
                    }.then(
                        argument("minutes", IntegerArgumentType.integer(0)).executes { ctx ->
                            handleTimeAction(ctx, action, true, false, false)
                        }.then(
                            argument("hours", IntegerArgumentType.integer(0)).executes { ctx ->
                                handleTimeAction(ctx, action, true, true, false)
                            }.then(
                                argument("days", IntegerArgumentType.integer(0)).executes { ctx ->
                                    handleTimeAction(ctx, action, true, true, true)
                                }
                            )
                        )
                    )
            ...
            literal("timer")
                ...
                .then(literal("restart").executes { context ->
                    ...
                    if (GlobalConfigManager.config.showCommandFeedback) {
                        context.source.sendFeedback(Component.literal("Timer restarted").withColor(0xd5deee))
                    }
                    Command.SINGLE_SUCCESS
                })
                .then(literal("start").executes { context ->
                    ...
                    TimerHudRenderer.isVisible = true
                    if (GlobalConfigManager.config.showCommandFeedback) {
                        context.source.sendFeedback(Component.literal("Timer started").withColor(0xd5deee))
                    }
                    Command.SINGLE_SUCCESS
                })
                .then(literal("pause").executes { context ->
                    TimerManager.pause()
                    if (GlobalConfigManager.config.showCommandFeedback) {
                        context.source.sendFeedback(Component.literal("Timer paused").withColor(0xd5deee))
                    }
                    Command.SINGLE_SUCCESS
                })
                .then(literal("continue").executes { context ->
                    ...
                    TimerHudRenderer.isVisible = true
                    if (GlobalConfigManager.config.showCommandFeedback) {
                        context.source.sendFeedback(Component.literal("Timer continued").withColor(0xd5deee))
                    }
                    Command.SINGLE_SUCCESS
                })
                .then(literal("hide").executes { context ->
                    ...
                    if (GlobalConfigManager.config.showCommandFeedback) {
                        context.source.sendFeedback(Component.literal("Timer hidden").withColor(0xd5deee))
                    }
                    Command.SINGLE_SUCCESS
                })
                .then(literal("show").executes { context ->
                    TimerHudRenderer.isVisible = true
                    if (GlobalConfigManager.config.showCommandFeedback) {
                        context.source.sendFeedback(Component.literal("Timer shown").withColor(0xd5deee))
                    }
                    Command.SINGLE_SUCCESS
                })
                .then(literal("config").executes { _ ->
    ...
    private fun sendHelp(source: FabricClientCommandSource) {
        ...
        val helpText = Component.literal(">>> Advanced Timer Help <<<").withColor(titleColor)
            ...
            .append(addHelpLine("/timer restart", "Restarts the timer"))
            .append(addHelpLine("/timer time <add|subtract|set> <sec> [min] [hr] [day]", "Modifies time"))
