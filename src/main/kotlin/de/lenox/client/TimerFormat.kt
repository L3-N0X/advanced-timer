    ...
    fun format(ms: Long): String {
        return ms.milliseconds.toComponents { days, hours, minutes, seconds, _ ->
            ...
            result = optionalBlockRegex.replace(result) { matchResult ->
                ...
                val needsShowing =
                    (inner.contains("d") && days > 0) || (inner.contains("h") && hours > 0) || (inner.contains("m") && minutes > 0) || (inner.contains(
                        "s"
                    ) && seconds > 0)
            ...
            // Replace time units
            result = result.replace("dd", days.toString().padStart(2, '0')).replace("d", days.toString())
                .replace("hh", hours.toString().padStart(2, '0')).replace("h", hours.toString())
                .replace("mm", minutes.toString().padStart(2, '0')).replace("m", minutes.toString())
                .replace("ss", seconds.toString().padStart(2, '0')).replace("s", seconds.toString())
